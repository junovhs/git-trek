...
fn test_load_repo_data_empty_repo() {
    let (_dir, repo) = create_test_repo();
    // No commits yet - this should fail gracefully or return empty
    let result = load_repo_data(&repo, 10);
    // Empty repo has no HEAD, should error or return empty
...
fn test_load_repo_data_tracks_file_history() {
    ...
    // Should have history at multiple commit indices
    assert!(file.history.len() >= 1);
}
...
fn test_restore_file_nested_path() {
    ...
    // Restore should recreate... actually restore_file just writes, doesn't create dirs
    // This tests current behavior
    let result = restore_file(&repo, oid, "src/lib/module.rs");
    
    // Current impl might fail if dir doesn't exist - that's a bug to know about
    if result.is_ok() {
        let restored = fs::read_to_string(&file_path).expect("read");
        assert_eq!(restored, "nested content\n");
    }
    // If it fails, that's useful info too - we've found an edge case
}