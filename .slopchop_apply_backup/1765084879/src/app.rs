...
use crate::{
    cli::Cli,
    git_ops::{
        self, check_if_dirty, do_autostash, format_oid, head_info, load_commits, new_session,
...
impl App {
    ...
    pub fn checkout(&mut self) -> Result<()> {
        ...
        let msg = if self.used_worktree {
            format!("Now at {} (worktree).", git_ops::format_oid(oid))
        } else {
            self.repo.set_head_detached(oid)?;
            self.repo.checkout_head(Some(CheckoutBuilder::new().force()))?;
            format!("Now at {}.\nTo return: git switch {}", git_ops::format_oid(oid), self.original_branch)
        };
...
pub use git_ops::{format_oid, format_summary, Point};