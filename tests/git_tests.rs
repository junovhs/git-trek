...
fn test_restore_file_writes_to_disk() {
    ...
    // restore_file writes relative to cwd, so cd into repo
    let original_dir = env::current_dir().expect("get cwd");
    env::set_current_dir(dir.path()).expect("cd to repo");
    
    let result = restore_file(&repo, oid, "test.rs");
...
fn test_restore_file_older_version() {
    ...
    let result = restore_file(&repo, oid1, "test.rs");
